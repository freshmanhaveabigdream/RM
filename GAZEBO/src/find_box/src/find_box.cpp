...
public:
    find_box(): Node("find_box")
    {
        ...
        publisher_cmd = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);
        sub_keyboard = this->create_subscription<geometry_msgs::msg::Twist>("turtle1/cmd_vel", 10, std::bind(&find_box::callback_keyboard, this, std::placeholders::_1));
        hsv = {
        ...
        std::int64_t is_key = 0;    // 是否引入键盘的参数，若引入键盘则停止自动检测
        this->declare_parameter<std::int64_t>("is_key", is_key);
    }
private:
    std::int64_t is_key;
    ...
    rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr publisher_cmd{nullptr};
    rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr sub_keyboard;  // 定义键盘指令订阅者
    // 根据目标颜色物体，处理图像
    ...
    void callback_camera(const sensor_msgs::msg::Image & msg){
        ...
        if(isFound){
            pub.angular.z = (pro_img.cols / 2 - center_x) * 0.005;   //角速度
            pub.linear.x = (300 - width) * 0.005; //线速度
        }else{
            pub.angular.z = 0.5;
            pub.linear.x = 0.5;
        }
        this->get_parameter("is_key", is_key);
        if(is_key != 0){
            pub.angular.z = 0;
            pub.linear.x = 0;
        }
        publisher_cmd->publish(pub);
        cv::waitKey(1);
    }
    void callback_keyboard(const geometry_msgs::msg::Twist &cmd){
