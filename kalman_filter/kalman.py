...
# 生成含有噪声的数据点
true_values = np.linspace(0, 300, 300)
noise = np.random.uniform(0.01, 0.1, 300)
flag = np.random.choice((-1, 1), 300)
values_with_noise = true_values + flag * true_values * noise

fig, axs = plt.subplots(3, 3, sharex=True, sharey=False, figsize=(14, 8))  # 创建合适的画布
...
for i in range(3):
    for j in range(3):
        ...
        x_0 = 0
        p_0 = 0  # 初始化后验估计协方差
        Q_cov = 0.5 * 10 ** -i  # 初始化过程噪声方差
        R_cov = 0.5 * 10 ** -j  # 初始化观测噪声方差
        kalman = kalman_filter(x_0, p_0, Q_cov, R_cov)
        filtered_estimates = kalman.filter(values_with_noise, 0)

        axs[i, j].plot(true_values, label='True Values')
        axs[i, j].plot(values_with_noise, label='values_with_noise')
